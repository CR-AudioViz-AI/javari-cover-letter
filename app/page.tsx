"use client";
// app/page.tsx — Javari Cover Letter
// CR AudioViz AI · EIN 39-3646201 · May 2026
import { useState } from "react";

const TOOLS = [{"icon": "\ud83d\udcc4", "label": "Cover Letter", "desc": "Tailored to any job posting", "href": "/write"}, {"icon": "\ud83d\udccb", "label": "Resume Review", "desc": "AI resume optimization", "href": "/resume"}, {"icon": "\ud83c\udfaf", "label": "Job Match", "desc": "Align skills to job requirements", "href": "/match"}, {"icon": "\ud83d\udcbc", "label": "LinkedIn About", "desc": "Compelling LinkedIn summary", "href": "/linkedin"}, {"icon": "\ud83e\udd1d", "label": "Thank You Note", "desc": "Post-interview follow-ups", "href": "/thankyou"}, {"icon": "\ud83d\udca1", "label": "Interview Prep", "desc": "Common questions and answers", "href": "/prep"}];
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          stream: false,
          systemOverride: "You are an expert career coach and professional writer. Write compelling, personalized cover letters that get candidates hired. Be specific, enthusiastic, and tailored.",
        }),
      });
    ...
    <div style={{ minHeight:"100vh", background:"#040912", color:"#e2e8f0", fontFamily:"system-ui" }}>
      <nav style={{ background:"#1E3A5F", padding:"0 20px", height:52, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ fontWeight:800, color:"#00B4D8", fontSize:15 }}>Javari Cover Letter</span>
        </div>
      ...
      <section style={{ background:"linear-gradient(135deg,#1E3A5F,#040912)", padding:"56px 24px 48px", textAlign:"center" }}>
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <h1 style={{ fontSize:"clamp(24px,4vw,44px)", fontWeight:900, color:"#fff", margin:"0 0 12px", lineHeight:1.05 }}>Cover Letters That<br /><span style={{ color:"#00B4D8" }}>Get Interviews</span></h1>
          <p style={{ color:"rgba(255,255,255,0.7)", fontSize:15, lineHeight:1.65, margin:0 }}>AI-crafted cover letters tailored to any job. Stand out from hundreds of applicants.</p>
        </div>
      </section>
      <section style={{ maxWidth:720, margin:"0 auto", padding:"28px 20px 0" }}>
        <div style={{ background:"#0F1F32", border:"1px solid rgba(0,180,216,0.12)", borderRadius:14, padding:"20px 24px" }}>
          <h2 style={{ margin:"0 0 12px", fontSize:15, fontWeight:700, color:"#fff" }}>Write My Cover Letter</h2>
          <div style={{ display:"flex", gap:8 }}>
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&generate()}
              placeholder="Job title: [title], Company: [company], My background: [brief background]"
